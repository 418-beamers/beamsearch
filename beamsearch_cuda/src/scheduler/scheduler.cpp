...
DecayScheduleGenerator::DecayScheduleGenerator(int t_res, int e_res, float max_e)
    : time_resolution(t_res), entropy_bins(e_res), max_entropy(max_e) {
  lut.resize(time_resolution * entropy_bins);
}

void DecayScheduleGenerator::generateSyntheticData() {
...
void DecayScheduleGenerator::saveToBinary(const std::string& filename) {
...
bool DecayScheduleGenerator::loadFromBinary(const std::string& filename) {
...
Params DecayScheduleGenerator::query(int current_t, const std::vector<float>& entropy_points, int min_beam, int max_beam) const {
